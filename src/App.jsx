...
// ===== simple ErrorBoundary (viser fejl på skærmen) =====
function ErrorBoundary({ children }) {
  const [err, setErr] = useState(null);
  useEffect(() => {
    const onErr = (e) => setErr(e.error || e.message || "Ukendt fejl");
    window.addEventListener("error", onErr);
    window.addEventListener("unhandledrejection", (e) =>
      setErr(e.reason?.message || String(e.reason))
    );
    return () => {
  ...
  if (err) {
    return (
      ...
      >
        <h2 style={{ marginTop: 0 }}>Der opstod en fejl</h2>
        <pre
          style={{
            whiteSpace: "pre-wrap",
            background: "#111",
        ...
        <div style={{ marginTop: 8, opacity: 0.85 }}>
          (Genindlæs siden efter du har rettet, eller slet app-ikonet og føj til
          hjemmeskærm igen for at rydde PWA-cache.)
        </div>
      </div>
    );
  }
...
// ===== helpers =====
function uuid() {
  // iOS <16 kan mangle randomUUID – fallback
...
async function getSignedUrl(path, ttlHours = 24 * 7) {
  ...
  const { data } = await supabase.storage
    .from("photos")
    .createSignedUrl(path, 60 * 60 * ttlHours);
...
export default function App() {
  ...
  // Vis login-boks indtil der er en session
  if (!user) {
    return (
      <ErrorBoundary>
        <LoginBox />
  ...
  async function loadBeers() {
    let q = supabase
      .from("beers")
      .select(
        "id, name, brewery, style, color, price, rating, photo_path, created_at"
      )
      .eq("club_id", CLUB_ID);

    if (search) {
      q = q.or(
        `name.ilike.%${search}%,brewery.ilike.%${search}%,style.ilike.%${search}%,color.ilike.%${search}%`
      );
    }

    const { data, error } = await q.order(sortBy, {
    ...
    const entries = await Promise.all(
      (data || []).map(async (b) => [b.id, await getSignedUrl(b.photo_path)])
    );
  ...
  async function addBeer() {
    ...
    if (draft.photoDataUrl) {
      ...
      const { error: upErr } = await supabase.storage
        .from("photos")
        .upload(photo_path, file, { contentType: "image/jpeg" });
      if (upErr) {
        alert("Upload fejlede: " + upErr.message);
        return;
      }
    }
  ...
  async function deleteBeer(b) {
    ...
    if (b.photo_path) {
      try {
        await supabase.storage.from("photos").remove([b.photo_path]);
      } catch {}
    }
    await loadBeers();
  }
      ...
      <div style={{ minHeight: "100vh", background: "#0f1115", color: "white" }}>
        <div style={{ maxWidth: 980, margin: "0 auto", padding: 24 }}>
          ...
          >
            ...
            <button onClick={signOut} style={btn("ghost-sm")}>
              Log ud
            </button>
          </div>
          ...
          <div
            style={{
              ...
              background: coverUrl
                ...
                : "#161922",
              display: "grid",
              placeItems: coverUrl ? "unset" : "center",
              color: "#8b8f9a",
            }}
          >
          ...
          >
            <button onClick={() => setAddOpen(true)} style={btn("primary")}>
              Tilføj Øl
            </button>
            <input
              value={search}
          ...
          >
            <span style={{ opacity: 0.8 }}>Sorter:</span>
            <select
              value={sortBy}
                    ...
                    <div style={{ display: "flex", flexWrap: "wrap" }}>
                      ...
                      <div style={{ flex: 1, minWidth: 200, padding: 12 }}>
                        ...
                        <div style={{ marginTop: 8 }}>
                          <Stars value={b.rating ?? 0} onChange={() => {}} />
                        </div>
                        <div
                          style={{
                            marginTop: 8,
          ...
          >
            ...
            >
              ...
              >
                ...
                <input
                  style={input()}
                  placeholder="Farve (fx Gylden / Amber / Mørk)"
                  value={draft.color}
              ...
              >
                ...
                <button onClick={addBeer} style={btn("primary")}>
                  Gem
                </button>
              </div>
            </div>
            ...
            >
              ...
              >
                ...
                <input
                  style={input()}
                  placeholder="Bryggeri"
                  value={editDraft.brewery}
                  onChange={(e) =>
                    setEditDraft({ ...editDraft, brewery: e.target.value })
                  }
                />
                <input
                  style={input()}
                  ...
                  onChange={(e) =>
                    setEditDraft({ ...editDraft, style: e.target.value })
                  }
                />
                <input
                  style={input()}
              ...
              >
                ...
                <button onClick={updateBeer} style={btn("primary")}>
                  Gem
                </button>
              </div>
            </div>
        ...
        {cropOpen && (
          ...
          >
            ...
            >
              ...
              >
                <span style={{ opacity: 0.8, fontSize: 14 }}>Zoom</span>
                <input
                  type="range"
                  min={1}
                  max={3}
                  ...
                  style={{ flex: 1 }}
                />
                <button onClick={() => setCropOpen(false)} style={btn("ghost-sm")}>
                  Annuller
                </button>
                <button onClick={confirmCrop} style={btn("primary")}>
                  Brug udsnit
                </button>
              </div>
            </div>
